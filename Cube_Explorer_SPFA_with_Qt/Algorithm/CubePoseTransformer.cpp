...
string CubePoseTransformer::ReTransform(string res, int8_t poseId)
{
	int len = res.length();
	string tmpRes = res;
	for (int i = 0; i < len; i++) {
		switch (res[i])
		{
			/*{F, R, U, B, L, D},*/
		case 'F':
			tmpRes[i] = faceLib[reTransform[poseId][0]];
			break;
		case 'R':
			tmpRes[i] = faceLib[reTransform[poseId][1]];
			break;
		case 'U':
			tmpRes[i] = faceLib[reTransform[poseId][2]];
			break;
		case 'B':
			tmpRes[i] = faceLib[reTransform[poseId][3]];
			break;
		case 'L':
			tmpRes[i] = faceLib[reTransform[poseId][4]];
			break;
		case 'D':
			tmpRes[i] = faceLib[reTransform[poseId][5]];
			break;
		default:
			break;
		}
