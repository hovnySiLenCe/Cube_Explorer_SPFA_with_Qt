...
string recognizeNew() { // ！！重要算法
	...
	sort(vec_samBlocks.begin(), vec_samBlocks.end(), [](const CubeBlock& a, const CubeBlock& b) {return a.meanS < b.meanS; });
	//再对H值进行排序，去最大的9个作为白色块，避免黄色块过亮产生影响
	//sort(vec_samBlocks.begin(), vec_samBlocks.begin()+10, [](const CubeBlock& a, const CubeBlock& b) {return a.meanH > b.meanH; });
	int i = 0;
