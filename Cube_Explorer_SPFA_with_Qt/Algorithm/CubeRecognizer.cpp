...
//固定参数法识别
string recognize() {
	//对四张拍摄图片依次读取并进行识别，识别结果存放到 map_face_id_recogRes 中
	cv::Mat image;

	//策略1--------------------------------------------------------------------------------
	image = cv::imread("pic_cam/cam_FR.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "FR", 1, 2);

	image = cv::imread("pic_cam/cam_U.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "U", 1, 1);

	image = cv::imread("pic_cam/cam_BL.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "BL", 1, 2);

	image = cv::imread("pic_cam/cam_D.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "D", 1, 1);

	image = cv::imread("pic_cam/cam_case2_FR.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "FR", 2, 2);

	image = cv::imread("pic_cam/cam_case2_U.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "U", 2, 1);

	image = cv::imread("pic_cam/cam_case2_BL.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "BL", 2, 2);

	image = cv::imread("pic_cam/cam_case2_D.jpg");
	cv::resize(image, image, cv::Size(640, 360));
	judgeColor(image, "D", 2, 1);
	//-------------------------------------------------------------------------------------
...
string recognizeNew() { // ！！重要算法
	...
	////对四张拍摄图片依次读取并进行采样，识别结果存放到 map_face_id_recogRes 中
	//cv::Mat image;

	//image = cv::imread("pic_cam/cam_FR.jpg");
	//cv::resize(image, image, cv::Size(640, 360));
	////sampleFromPic(image, "UF", 2);
	//sampleFromPic(image, "FR", 2);

	//image = cv::imread("pic_cam/cam_U.jpg");
	//cv::resize(image, image, cv::Size(640, 360));
	//sampleFromPic(image, "U", 1);
	////sampleFromPic(image, "R", 1);

	//image = cv::imread("pic_cam/cam_BL.jpg");
	//cv::resize(image, image, cv::Size(640, 360));
	//sampleFromPic(image, "BL", 2);
	////sampleFromPic(image, "DB", 2);

	//image = cv::imread("pic_cam/cam_D.jpg");
	//cv::resize(image, image, cv::Size(640, 360));
	////sampleFromPic(image, "L", 1);
	//sampleFromPic(image, "D", 1);
